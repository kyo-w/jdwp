...
type Reader interface {
	...
	String() string
	// Count Decode a collection count from the stream.
	Count() uint32
	// Error If there is an error reading any input, all further reading returns the
	// zero value of the api read. Error() returns the error which stopped
	// reading from the stream. If reading has not stopped it returns nil.
	Error() error
	// SetError Set the error state and stop reading from the stream.
	SetError(error)
}
