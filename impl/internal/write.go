...
type Writer interface {
	...
	String(string)
	// Error If there is an error writing any output, all further writing becomes
	// a no-op. Error() returns the error which stopped writing to the stream.
	// If writing has not stopped it returns nil.
	Error() error
	// SetError Set the error state and stop writing to the stream.
	SetError(error)
}
...
func Write(w Writer, v interface{}) {
	...
	case reflect.String:
		w.String(r.String())
	default:
		panic(fmt.Errorf("Cannot write api %T", v))
	}
}
